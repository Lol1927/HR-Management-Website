...
import { X, Star, Save, CheckCircle2, UserCircle } from 'lucide-react';
...
function EventEvaluationFullModal({ event, onClose, API_URL }) {
  ...
  const handleSaveAll = async () => {
    try {
      // API 설계에 따라 evaluations 전체를 보내거나, 각각 보냅니다.
      await axios.post(`${API_URL}/evaluations/bulk`, {
        eventId: event.id,
        evaluations: evalData
      });
      alert("모든 인력 평가가 저장되었습니다.");
      onClose();
    } catch (err) {
  ...
  return (
    <div className="fixed inset-0 bg-white z-[500] flex flex-col animate-in fade-in duration-300">
      ...
      <header className="p-6 border-b bg-slate-50 flex justify-between items-center">
        ...
        <button onClick={onClose} className="p-4 bg-white rounded-full text-slate-400 hover:text-slate-800 shadow-sm border transition-all">
          <X size={24} />
        </button>
      </header>
            ...
            <div className="max-w-3xl mx-auto space-y-12">
              ...
              <section className="space-y-4">
                ...
                <textarea 
                  ...
                  className="w-full h-64 p-8 bg-slate-50 border-2 border-slate-100 rounded-[40px] focus:bg-white focus:border-blue-500 focus:ring-4 focus:ring-blue-50 focus:outline-none transition-all text-slate-700 font-medium leading-relaxed"
                />
              </section>
              ...
              <button 
                ...
                className="w-full py-6 bg-slate-900 text-white rounded-[32px] font-black text-xl hover:bg-blue-600 hover:shadow-2xl hover:shadow-blue-200 transition-all flex items-center justify-center gap-3"
              >
                <Save size={24} /> 전체 인원 평가 결과 제출하기
              </button>
            </div>
          ) : (
