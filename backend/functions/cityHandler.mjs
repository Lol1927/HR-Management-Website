...
function WorkplaceManagement({ API_URL }) {
  ...
  const addProvince = async () => {
    ...
    try {
      const res = await axios.post(`${API_URL}/provinces`, { provinceName: newProvinceName });
      if (res.data.error) {
  ...
  const deleteProvince = async (name) => {
    ...
    try {
      await axios.delete(`${API_URL}/provinces/${encodeURIComponent(name)}`);
      await fetchData();
    } catch (err) {
      alert("삭제 실패");
    }
  ...
  const handleCitySubmit = async () => {
    ...
    try {
      const res = await axios.post(`${API_URL}/cities`, { 
  ...
  const deleteCity = async (cityName) => {
    ...
    try {
      await axios.delete(`${API_URL}/cities/${encodeURIComponent(cityName)}`);
      await fetchData();
    } catch (err) {
      alert("시 삭제 실패");
    }
